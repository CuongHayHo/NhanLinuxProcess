...
6.  **Shell Manager**: Thực thi lệnh, chạy script `/bin/bash`, quản lý biến môi trường, tự động hóa tác vụ (Automation/Cron) và cấu hình thời gian (Time Configuration: cấu hình thủ công qua lệnh `date -s` với quyền root — Learning Mode hiển thị API POSIX tương đương `clock_settime()`/`CAP_SYS_TIME`, đồng bộ thời gian qua Internet sử dụng chronyc/ntpdate, và cấu hình đồng bộ tự động NTP sử dụng timedatectl).
